...
<div class="panel" id="files"><h3>文件管理</h3><p class="hint">上传并管理后台文件。支持 jpg/png/webp/gif/pdf/mp4，最大 200MB。</p><div class="filebox"><input type="file" id="fileInput"><button class="primary" id="uploadBtn" onclick="uploadFile()">上传文件</button></div><div id="uploadStatus" style="font-size:13px;color:var(--muted);margin-bottom:10px;display:none"></div><div class="table-wrap"><table id="fileTable"><thead><tr><th>文件名</th><th>大小</th><th>链接</th></tr></thead><tbody></tbody></table></div></div>
...
<script>
let TOKEN="";
function api(path,opt={}){return fetch(path,{...opt,credentials:'same-origin'}).then(async r=>{if(r.ok)return r.json();let errMsg='请求失败 ('+r.status+')';try{let e=await r.json();errMsg=e.detail||e.error||errMsg}catch(_){}throw new Error(errMsg)}).catch(e=>{if(!e||!e.message)e=new Error('网络错误，请检查连接');alert(e.message);if('Unauthorized'===e.message||e.message.includes('401')){document.getElementById('login').style.display='block';document.getElementById('app').style.display='none'}throw e})}
...
async function loadContacts(){let rows=await api('/api/admin/contacts');let html=rows.map(r=>{let btnStyle=r.hermes_reply?'':'background:linear-gradient(135deg,#7c3aed,#06b6d4);color:#fff;border:none';let btnText=r.hermes_reply?'重新生成':'AI 回复';let safeMsg=r.message.replace(/</g,'&lt;').replace(/>/g,'&gt;');let replyRow='';if(r.hermes_reply){let safeReply=r.hermes_reply.replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');replyRow=`<tr class="reply-row"><td colspan="6"><div class="reply-label">💬 Hermes 回复建议</div><div class="reply-content">${safeReply}</div><div class="reply-actions"><button class="btn small" onclick="copyReply(${r.id})">复制</button></div></td></tr>`}return `<tr><td>${r.id}</td><td>${r.name}</td><td>${r.email}</td><td style="max-width:320px">${safeMsg}</td><td>${r.created_at?.slice(0,16)||''}</td><td style="white-space:nowrap"><button class="btn" onclick="replyContact(${r.id})" style="${btnStyle}">${btnText}</button> <button class="btn danger" onclick="delContact(${r.id})">删除</button></td></tr>${replyRow}`}).join('');document.querySelector('#contactTable tbody').innerHTML=html||'<tr><td colspan="6">暂无留言</td></tr>'}
...
async function uploadFile(){let f=document.getElementById('fileInput').files[0];let btn=document.getElementById('uploadBtn');let status=document.getElementById('uploadStatus');if(!f){alert('请先选择要上传的文件');return;}if(f.size>200*1024*1024){alert('文件超过 200MB 限制');return;}btn.disabled=true;btn.textContent='上传中...';status.style.display='block';status.style.color='var(--muted)';status.textContent='正在上传 '+f.name+' ('+(f.size/1024/1024).toFixed(1)+'MB)...';try{let fd=new FormData();fd.append('file',f);let r=await api('/api/admin/upload',{method:'POST',body:fd});loadFiles();status.style.color='var(--ok)';status.textContent='上传成功！链接: '+r.url;alert('上传成功！\n链接: '+r.url)}catch(e){status.style.color='var(--danger)';status.textContent='上传失败: '+e.message}finally{btn.disabled=false;btn.textContent='上传文件'}}
